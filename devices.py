...
"border-style: inset;\n"
"}")
        searchCondision = ['按设备名称查询','按设备号查询',  '按购买日期查询']
...
" alternate-background-color: rgb(240,195,186); /*行交替颜色*/  \n"
" selection-background-color: rgb(117,207,219); /*选中行背景颜色*/ \n" 
"}  \n"
...
"}  \n"
        ...
        self.queryModel.setTable("devices")
        ...
        self.queryModel.setHeaderData(1, Qt.Horizontal, "设备名称")
        self.queryModel.setHeaderData(2, Qt.Horizontal, "购买日期")
        self.queryModel.select()
    ...
    def recordQuery(self, index):
        ...
        if (conditionChoice == '按设备名称查询'):
            conditionChoice = 'dev_name'
            self.input= self.searchEdit.text()
            self.allCondition=conditionChoice+"='"+str(self.input)+"'"
        elif (conditionChoice == '按设备号查询'):
            conditionChoice = 'dev_id'
            self.input= self.searchEdit.text()
            self.allCondition=conditionChoice+"="+str(self.input)
        elif (conditionChoice == '按购买日期查询'):
            conditionChoice = 'purchase_date'
            self.input= self.searchEdit.text()
            self.allCondition=conditionChoice+"='"+str(self.input)+"'"
      

        if (self.searchEdit.text() == ""):
            self.queryModel.setFilter("dev_id>0")
            self.queryModel.select()
            self.totalRecord = self.queryModel.rowCount()
            self.totalPage = int((self.totalRecord + self.pageRecord - 1) / self.pageRecord)
