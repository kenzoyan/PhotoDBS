...
class picgroup(QWidget):
    def __init__(self):
        super(picgroup, self).__init__()
        self.resize(700, 500)
        self.setWindowTitle("图片分组管理")
        # 查询模型
...
"border-style: inset;\n"
"}")
        searchCondision = ['按分组名称查询','按分组编号查询',  '按分组人数查询']
...
"}  \n"
        ...
        self.queryModel.setTable("pic_group")
    ...
    def recordQuery(self, index):
        ...
        if (conditionChoice == '按分组名称查询'):
            conditionChoice = 'picgroup_name'
            self.input= self.searchEdit.text()
            self.allCondition=conditionChoice+"='"+str(self.input)+"'"
        elif (conditionChoice == '按分组编号查询'):
            conditionChoice = 'picgroup_id'
            self.input= self.searchEdit.text()
            self.allCondition=conditionChoice+"="+str(self.input)
        elif (conditionChoice == '按分组人数查询'):
            conditionChoice = 'picgroup_number'
            self.input= self.searchEdit.text()
            self.allCondition=conditionChoice+"="+str(self.input)
      

        if (self.searchEdit.text() == ""):
            self.queryModel.setFilter("picgroup_id>0")
            self.queryModel.select()
            self.totalRecord = self.queryModel.rowCount()
            self.totalPage = int((self.totalRecord + self.pageRecord - 1) / self.pageRecord)
