...
class photographers(QWidget):
    def __init__(self):
        super(photographers, self).__init__()
        self.resize(1000, 800)
        self.setWindowTitle("摄影师管理")
        # 查询模型
...
"border-style: inset;\n"
"}")
        searchCondision = ['按摄影师姓名查询','按摄影师编号查询',  '按年龄查询', '按小组查询']
...
"}  \n"
        ...
        self.queryModel.setTable("photographers")
        #利用外键 所在小组
        self.queryModel.setRelation(5, QSqlRelation("people_group", "peogroup_id", "peogroup_name"))
        ...
        self.queryModel.setHeaderData(0, Qt.Horizontal, "摄影师编号")
        self.queryModel.setHeaderData(1, Qt.Horizontal, "名字")
        self.queryModel.setHeaderData(2, Qt.Horizontal, "性别")
        self.queryModel.setHeaderData(3, Qt.Horizontal, "年龄")
        self.queryModel.setHeaderData(4, Qt.Horizontal, "电话号码")
        self.queryModel.setHeaderData(5, Qt.Horizontal, "所在小组")
        self.queryModel.select()
    ...
    def recordQuery(self, index):
        ...
        if (conditionChoice == '按摄影师姓名查询'):
            conditionChoice = 'name'
            self.input= self.searchEdit.text()
            self.allCondition=conditionChoice+"='"+self.input+"'"
        elif (conditionChoice == '按摄影师编号查询'):
            conditionChoice = 'photographers.people_id'
            self.input= self.searchEdit.text()
            self.allCondition=conditionChoice+"="+self.input

        elif (conditionChoice == '按年龄查询'):
            conditionChoice = 'age'
            self.input= self.searchEdit.text()
            self.allCondition=conditionChoice+"="+self.input

        elif (conditionChoice == '按小组查询'):
            conditionChoice = 'photographers.peogroup_id'
            self.input= self.searchEdit.text()
            query.exec("select people_group.peogroup_id from people_group where people_group.peogroup_name='%s'"%self.input)
            query.first()
        ...
        if (self.searchEdit.text() == ""):
            ...
            self.queryModel.setFilter("photographers.people_id>0")
            self.queryModel.select()
            self.totalRecord = self.queryModel.rowCount()
            self.totalPage = int((self.totalRecord + self.pageRecord - 1) / self.pageRecord)
